...
import { LuClock as Clock,LuCalendar as CalendarIcon, LuChevronLeft as ChevronLeft, LuChevronRight as ChevronRight } from  "react-icons/lu";
...
function Calendar({
  ...
  const _classNames:any = {
    ...
    nav: 'space-x-1 flex justify-between  w-[14rem] -mt-5',
    button_previous: cn(
    ...
    day: 'h-9 w-9 text-center text-sm p-0 relative [&:has([aria-selected].day-range-end)]:rounded-r-md [&:has([aria-selected].day-outside)]:bg-slate-400/50 [&:has([aria-selected])]:bg-slate-400 first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20 rounded-1',
    day_button: cn(
    ...
    today: 'bg-slate-400 text-accent-foreground',
    outside:
      'day-outside text-muted-foreground opacity-50 aria-selected:bg-slate-400/50 aria-selected:text-muted-foreground aria-selected:opacity-30',
    disabled: 'text-muted-foreground opacity-50',
    range_middle: 'aria-selected:bg-slate-400 aria-selected:text-accent-foreground',
    hidden: 'invisible',
    ...classNames,
  }
    ...
    MonthCaption: ({ calendarMonth }) => {
      return (
        <div className="inline-flex gap-2">
          ...
          >
            <SelectTrigger className="w-fit gap-1 border-none p-0 focus:bg-slate-400 focus:text-accent-foreground">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {MONTHS.map((month) => (
    ...
    return (
      <div className="flex h-10 items-center">
        <Select defaultValue={period} onValueChange={(value: Period) => handleValueChange(value)}>
          <SelectTrigger
            ref={ref}
            className="w-[65px] focus:bg-slate-400 focus:text-accent-foreground"
  ...
  (
    ...
    return (
      <Input
        ...
        name={name || picker}
        className={cn(
          'w-[48px] text-center font-mono text-base tabular-nums caret-transparent focus:bg-slate-400 focus:text-accent-foreground [&::-webkit-inner-spin-button]:appearance-none',
          className,
        )}
